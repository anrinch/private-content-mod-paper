...
\item If \server aborts, then the simulator aborts. Otherwise if \server sends $\partitionIDSet$, the simulator computes and sends to \server $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\genericCInputElmt)}$, where $\genericRand_\partitionIdx \sample [1, \rsaMod^2]$, and $R_\partitionIdx \sample \bin^{\secParam}$ for $\partitionIdx \in \partitionIDSet$. The simulator stores $\langle \genericRand_\partitionIdx, R_\partitionIdx \rangle$
\item The simulator answers queries to the random oracle \ro for \server. If \server sends a query for $\qrGen^{\genericRand_\partitionIdx}$ for some $\genericRand_\partitionIdx$ used in the previous step, then the simulator responds to the query by outputting $\fileKey \oplus R_\partitionIdx$. Otherwise, the simulator responds with a random element $\bin^{\secParam}$ and stores the output
...
\item If \server aborts, then the simulator abort. Otherwise, receive $\partitionIDSet$ from the adversary. 
\item The simulator samples from $\hat{\genericCInputElmt} \notin \blocklist$
\item For each $\partitionIdx \in \partitionIDSet$, the simulator computes and sends to \server $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\hat{\genericCInputElmt})}$, where $\genericRand_\partitionIdx \sample [1, \rsaMod^2]$, and $R_\partitionIdx \sample \bin^{\secParam}$ for $\partitionIdx \in \partitionIDSet$.  The simulator stores $\langle \genericRand_\partitionIdx, R_\partitionIdx \rangle$
\item The simulator respond to queries to \ro from \server. If there is a query for $\qrGen^{\genericRand_\partitionIdx}$ for some $\genericRand_\partitionIdx$ selected in the previous step, then the simulator aborts. Otherwise, the simulator responds with a random element $\bin^{\secParam}$ and stores the output
\item The simulator outputs whatever \server outputs
\end{enumerate}
\end{enumerate}
\end{itemize}
...
\noindent \underline{Type 1 output}: In this case, \server's messages includes $ \prf{\prfKeyClient}(\genericCInputElmt)$, $\encrypt[\bloomFilterKey](\bloomFilter[\bloomFilterIdx])$, $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\genericCInputElmt)}$, $\ro(\qrGen^{\genericRand_\partitionIdx}) \oplus \fileKey $ in the real execution for all $\partitionIdx \in \partitionIDSet$. In the ideal world, \server's messages includes $\genericCInputElmtPRFEval$, $\encrypt[\bloomFilterKey](\bloomFilter[\bloomFilterIdx])$, $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\genericCInputElmt)}, R_\partitionIdx$ for all $\partitionIdx \in \partitionIDSet$. Assuming that \prf produces outputs indistinguishable from random  $ \prf{\prfKeyClient}(\genericCInputElmt)$ and $\genericCInputElmtPRFEval$ are indistinguishable to the adversary. Also, assuming that \ro is a random oracle $\ro(\qrGen^{\genericRand_\partitionIdx}) \oplus \fileKey $ is indistinguishable from $R_\partitionIdx$ unless the adversary has queried the random oracle on \ro on $\genericRand_\partitionIdx$ before which happens with negligible probability for a fresh randomness $\genericRand_\partitionIdx$ used in every query, since $\qrGen^{\genericRand_\partitionIdx}$ is a random element in \qrGroup~\cite{cramer2000signature}.  
...
When the protocol performs a fine-grained check, \server's messages includes $ \prf{\prfKeyClient}(\genericCInputElmt)$, $\encrypt[\bloomFilterKey](\bloomFilter[\bloomFilterIdx])$, $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\genericCInputElmt)}, \ro(\qrGen^{\genericRand_\partitionIdx}) \oplus \fileKey$ such that $\genericCInputElmt \notin \blocklist$. In the
simulation, \server's messages include $\hat{\genericCInputElmtPRFEval}$, $\encrypt[\bloomFilterKey](\bloomFilter[\bloomFilterIdx])$, $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\hat{\genericCInputElmt})}, R_\partitionIdx$ for all $\partitionIdx \in \partitionIDSet$. Observe that when $\genericCInputElmt \notin \blocklist$, the adversary does not know $\prf{\prfKeyClient}(\genericCInputElmt)$ in the real execution from the preprocessing phase. Thus, assuming that the \prf produces indistinguishable outputs from random $\prf{\prfKeyClient}(\genericCInputElmt)$ is indistinguishable from $\hat{\genericCInputElmtPRFEval}$. Also, due to \ro being a random oracle $\ro(\qrGen^{\genericRand_\partitionIdx}) \oplus \fileKey$ is indistinguishable from $R_\partitionIdx$ as discussed above. Finally, if the adversary can distinguish between $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\genericCInputElmt)}$ and $\genericSig[\partition{\partitionIdx}]^{\genericRand_\partitionIdx \primeRO(\hat{\genericCInputElmt})}$ when $\genericCInputElmt, \hat{\genericCInputElmt} \notin \blocklist$, then it violates the IND-SR property of the signature scheme, which happens with negligible probability as per \thmref{thm:indsr}. 
