...
remote services, e.g., public clouds, by the end of 2025~\cite{public_storage_vol}. 
...
mechanisms to identify such harmful content~\cite{apple2021csam,
  googleCSAM, amazonCSAM}. Typically, these services use blocklists curated by trusted agencies, e.g., the National Center for Missing \& Exploited Children (NCEMC). 


However, the task is more challenging with end-to-end encryption (E2EE)~\cite{content_mod_e2ee} because it precludes the service provider from implementing simple content sanitization mechanisms that has proved reasonably effective (so far) as a deterrent and an effective method to catch ``easy'' cases of proliferation~\cite{}\footnote{Of course, even if a service does not explicitly support end-to-end encryption, a motivated attacker can still store and distribute CSAM by encrypting the content; it is unclear whether any satisfactory solution can solve this problem without completely violating client-side privacy.}. This has raised serious concerns among child safety groups that rely on tips from service providers to detect proliferation of CSAM~\cite{}. 

To mitigate this tension, in 2021, Apple proposed a mechanism  
to detect harmful content in end-to-end encrypted photos uploaded to iCloud. The system, called the Apple PSI~\cite{bhowmick2021apple}, leveraged Apple's proprietary perceptual hashing technology, NeuralHash, to generate a hash of the content. The hash is then compared privately against a blocklist curated by Apple. 
If there is a match, then the iCloud server learns the decryption key. Sophisticated attackers can still circumvent detection by obfuscating their image, e.g., by ``double-encrypting'' it before uploading to iCloud, or tamper with the client-side software itself. Nonetheless, the Apple PSI mechanisms provided several important safeguards: i) it allowed the service provider to perform basic content sanitization while supporting E2EE, ii) it could act as a detterrant against bulk uploads by unsophisticated clients~\cite{}, and iii) it relieves the service provider of legal liabilities due to publicly hosting CSAM. 

Ultimately, the scheme was not deployed due to obvious concerns that it could be misused for oversight and surveillance~\cite{applePSIabandon}. Indeed, Apple's involvement in creating the
blocklist, and the lack of mechanisms that allow trusted parties, e.g., NCEMC, to authenticate the blocklist, is problematic~\cite{applePSICriticBBC,
  applePSICriticBU}. Second, the mechanism was tied exclusively to NeuralHash, which can easily
produce false positives~\cite{neuralHashAttack}; unfortunately, adapting the system to other kinds of perceptual hashes is non-trivial. While the first problem can be addressed through cryptographic mechanisms~\cite{}, the latter remains unaddressed to the best of our knowledge. 


In light of the discussion above, in this paper we are interested in resolving the following technical question 

\begin{myquote}
Can we design a content moderation scheme for end-to-end encrypted data stores with minimal trust assumptions that allows service providers to perform the same level of content sanitization as possible today without end-to-end to encryption?
\end{myquote}
...
%
We envision a typical setting where a group of independent parties (auditors) with semi-honest majority, e.g., law enforcement, and child safety groups, publish a common (obfuscated) blocklist. Such blocklists are
not uncommon, e.g., the NCEMC and FBI have published lists of missing
children and persons~\cite{ncemc}. We 
expect that a threshold number of these independent auditors will behave honestly, and posit that having a common blocklist curated by independent agencies, will build more trust in the system
than individual blocklists published by self-interested service
providers. 


Technically, the common blocklist is published as an unforgeable but publicly verifiable
...
takes roughly 430 ms for a blocklist with million elements. Assuming that realistically less than 10\% of the content would require a full check, because uploading CSAM is atypical user behavior, our protocol on average takes 43ms, \textbf{12$\times$ faster} than the state-of-the-art~\cite{kulshrestha2021identifying} \emph{that operates under a more restrictive threat model with a semi-honest server}. Our protocol also has \textbf{50$\times$} lower communication costs. 
...
Fuzzy matching takes less than 600ms for the fast path check, and 61 seconds for a full check against a blocklist with 1 million items, with false-negative rate of $< 0.08$. The fast path is \textbf{5$\times$} faster than the state-of-the-art~\cite{kulshrestha2021identifying} with lower overall communications costs by a factor of \textbf{25$\times$}.
...
%
The primary motivation of this work is to develop new cryptographic mechanisms for content moderation, but we also acknowledge the limitations and risks of deploying such a system. First, the efficacy of any content moderation system that only relies on server-side checks (as with our work and all existing designs) is limited to detecting content that is not obfuscated by the client before uploads e.g., by locally encrypting the data. This excludes a sizable set of attackers. However, evidence shows that even this basic level of monitoring is important for slowing down proliferation of CSAM~\cite{}. Service providers today are limited to only identifying unencrypted harmful content, as a motivated attacker can still provide an encrypted input even if the service does not support E2EE. The technical solutions in this paper extend these monitoring capabilities while also allowing the service provider to support end-to-end encryption. 

Second, as with any content moderation system, there are risks of misuse for surveillance. Technically, our solution addresses this problem by distributing trust among multiple independent agencies, but we acknowledge that this may not suffice against a powerful adversary, e.g., a nation state who may corrupt all the auditors. Thus, further discussions and potential mitigations are necessary before any content moderation system, including ours, is widely deployed. These discussions are out of scope for this paper. 
