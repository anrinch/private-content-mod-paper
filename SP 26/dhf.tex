...
\label{sec:dhf}
The Apple PSI system introduced a \emph{detectable hash function}. The following construction is adapted from their white paper, as detailed in \cite{pinkas2021applepsi}. A detectable hash function is an s-detectable t-wise independent hash function $\dhf: \dhfKeySpace \times \dhfDomain \rightarrow \dhfRange$, also called an $(s,t)$-DHF.
...
%
This problem is related to decoding the interleaved Reed-Solomon code under random noise, and can be solved using a technique described by \citet{coppersmith2003reconstructing} 
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Let $\mathbf{w} \in \field^{m}_{\ell}$ be a vector such that $M \cdot \mathbf{w} = 0$.
        \item Let $Z \subseteq [1,m]$ be the set of indices where $\mathbf{w} \neq 0$.
...
The correctness of the algorithm is implied by the construction in \cite{coppersmith2003reconstructing}. The algorithm has $\bigO((s+t) * m^2) \in \bigO(s^3)$ cost since it requires a matric decomposition over $M$. 
