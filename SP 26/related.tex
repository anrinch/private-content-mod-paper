...
\caption{\centering \small Encrypted content moderation systems. 
 $\blocklistSize$: blocklist size, $\epsilon \in (0,1/2)$, $\genericVecLen$: vector length, SH: semi-honest, M: malicious, Avg: average case cos, WC: worst case cost}
  \label{tab:related}
...
\cline{1-9}
 & Private Hash Matching~\cite{kulshrestha2021identifying} & N/A & SH & SH & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(1)$ &  $\bigO(\blocklistSize)$  \\
\cline{2-9}
Exact & Apple PSI~\cite{bhowmick2021apple} + Agg. Signature~\cite{scheffler2023public} & \checkmark & M & SH & $\bigO(1)$  & $\bigO(1)$ & $\bigO(\blocklistSize)$ & $\bigO(1)$   \\ 
\cline{2-9}
...
\cline{1-9}
 & Private Hash Matching~\cite{kulshrestha2021identifying} & N/A & SH & SH & $\bigO(\sqrt{\blocklistSize} \genericVecLen)$ & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(1)$ &  $\bigO(\blocklistSize  \genericVecLen)$    \\
%\cline{2-9}
...
\cline{2 - 9}
Fuzzy & {\bf This work} & \checkmark & M & SH & $\bigO(\blocklistSize^{\epsilon})$  & $\bigO(\blocklistSize^{\epsilon})$ & $\bigO(\blocklistSize^{1 + \epsilon})$ & \makecell{$\bigO(\blocklistSize^{\epsilon} \times \genericVecLen)$}    \\ 
\cline{1-9}
\end{tabular}
\vspace{0.2em}
\end{table*}
