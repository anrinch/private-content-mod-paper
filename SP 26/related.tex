...
\caption{\centering \small Encrypted content moderation systems.\\ 
 $\blocklistSize$: blocklist size, $\epsilon \in (0,1/2)$, $\genericVecLen$: vector length, SH: semi-honest, M: malicious, Avg: average-case cost, WC: worst-case cost}
  \label{tab:related}
\begin{tabular}{| c | c | c | c | c | c | c | c | c|}
\noalign{\global\arrayrulewidth1pt}\hline\noalign{\global\arrayrulewidth0.4pt}
\multirow{2}{*}{Type of Matching} & \multirow{2}{*}{System Name} & \multirow{2}{*}{Audibility} & \multicolumn{2}{c|}{Threat model} & \multicolumn{4}{c|}{Asymptotic Costs} \\
\cline{4 - 9} 
  & & & Server & Client & Client Comp. & Comm. Volume & Client Storage  & Server Comp. \\
\cline{1-9}
\multirow{4}{*}{Exact} & Private Hash Matching~\cite{kulshrestha2021identifying} & N/A & SH & SH & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(1)$ &  $\bigO(\blocklistSize)$  \\
\cline{2-9}
& Apple PSI~\cite{bhowmick2021apple} + Agg. Signature~\cite{scheffler2023public} & \checkmark & M & SH & $\bigO(1)$  & $\bigO(1)$ & $\bigO(\blocklistSize)$ & $\bigO(1)$   \\ 
\cline{2-9}
%& {\bf This work} (opt. client storage) & \checkmark & M & SH & $\bigO(1)$  & $\bigO(1)$ & $\bigO(1)$ & $\bigO(\blocklistSize \log \blocklistSize)$  \\ 
%\cline{2-9}
& {\bf This work} & \checkmark & M & SH & \makecell{Avg: $\bigO(1)$\\ WC: $\bigO(\log \blocklistSize)$}   & $\bigO(\blocklistSize^{\epsilon})$ & $\bigO(\blocklistSize^{\epsilon})$ & \makecell{Avg: $\bigO(1)$\\WC: $\bigO(\blocklistSize)$}\\ 
\cline{1-9}
\multirow{2}{*}{Fuzzy} & Private Hash Matching~\cite{kulshrestha2021identifying} & N/A & SH & SH & $\bigO(\sqrt{\blocklistSize} \genericVecLen)$ & $\bigO(\sqrt{\blocklistSize})$ & $\bigO(1)$ &  $\bigO(\blocklistSize  \genericVecLen)$    \\
%\cline{2-9}
...
\cline{2 - 9}
& {\bf This work} & \checkmark & M & SH & $\bigO(\blocklistSize^{\epsilon})$  & $\bigO(\blocklistSize^{\epsilon})$ & $\bigO(\blocklistSize^{1 + \epsilon})$ & \makecell{$\bigO(\blocklistSize^{\epsilon} \times \genericVecLen)$}    \\ 
\cline{1-9}
\end{tabular}
\vspace{0.2em}
\end{table*}
...
excellent survey \cite{scheffler2023sok}.
...
scheme also does not allow independent auditors to verify the blocklist.

% In contrast, we do 
% not assume any trust on the service provider. Finally, the approximate match scheme 
% only works for a specific type of content embedding with empirically determined parameters and high false negatives ($\approx$ 17\%). 
% The scheme cannot be trivially extended to other types of embeddings. 
...
leveraging an aggregated signature scheme. \emph{Nonetheless, the mechanism does not generalize to other types of content hashes, including ones that require fuzzy matching}. This is a drawback because NeuralHash without fuzzy matching is easy to circumvent, and also suffers from false positives~\cite{struppek2022learning, neuralHashAttack}. 

%We design protocols for exact and fuzzy matching independent of the content hashing technology. 
