...
%A \projectionPrimitiveIt is implied by a construction due to 
%\citet{chongchitmate2024approximate}. We formalize the primitive below 
%\mkr{Something here isn't right.  What is \projIdx?  And \hashFn takes one input, not two.}

%
%A \projectionPrimitiveIt is defined over a set $\blocklist
%\subseteq \metricSpace$ where \metricSpace is a metric space with
%distance $\distanceNotation[\distance][\cdot]$.
...
%
\corrref{eq:false_pos_bound} and \corrref{eq:false_neg_bound} show the relationship between the false positive rate and false negative rate with the detection threshold \detectionThreshold. Using the mechanism of \cite{chongchitmate2024approximate}, we can ensure that projections of the blocklist vectors uniquely identify them. Combining these two facts, we will build an encrypted index with the projections. Specifically, similar to the exact match protocol, in the fuzzy match protocol (\figref{fig:fuzzy_match}), the auditors will partition the blocklist vectors into \numBuckets disjoint partitions $\bucket{1}, \dots, \bucket{\numBuckets}$. The blocklist vector-to-partition identifier is stored in a cuckoo hash table \cuckooTable of size $\blocklistSize \numProjections(1+\cuckooTableParam)$ with hash functions $\cuckooHashFn{1}, \cuckooHashFn{2}$. If a vector $\blocklistVec \in \bucket{\bucketIdx}$, then for all $\projIdx \in \nats[1, \numProjections]$, either the location $\cuckooHashFn{1}(\genericSetVar[\projIdx][\blocklistVec])$ or $\cuckooHashFn{2}(\genericSetVar[\projIdx][\blocklistVec])$ in \cuckooTable store two values: the projection $\projFn[\projIdx](\blocklistVec)$ and the partition identifier $\bucketIdx$. The empty locations in \cuckooTable are padded with encryptions of random items in \group (\stepsref{fuzzy:pre:cht1}{fuzzy:pre:cht3}). 
...
\item $\bigO(\blocklistSize\genericVecLen/\numBuckets \log \blocklistSize\genericVecLen/\numBuckets )$ server cost when the client's uploaded file matches an item in the blocklist (fine-grained check)
