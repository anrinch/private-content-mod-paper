...
\newcommand{\bucketIDSet}{\ensuremath{\mathcal{I}}\xspace}
...
\item Auditors randomly partition the blocklist into $\numBuckets$ buckets, $\blocklist = \bucket{1} \cup  \dots \cup \bucket{\numBuckets}$
...
\item Auditors sample $\numBuckets$ random elements $\blindingFactor_1, \dots, \blindingFactor_{\numBuckets} \sample [1, \rsaMod^2]$ and jointly invert the elements to obtain additive shares of $\blindingFactor_{\bucketIdx}^{-1} \bmod \orderFn(\qrGroup)$ using \cite{catalano2000computing}. 


%\item Auditors sample $\numBuckets$ random prime numbers $\blindingFactor_1, \dots, \blindingFactor_{\numBuckets}$ and add $\blindingFactor_\bucketIdx$ to $\bucket{\bucketIdx}$ as blinding factors to hide blocklist elements from \client


%publish $\blocklist', \genericSig[\blocklist']$. 

\item Using $\distSigningFunc$ auditors jointly sign each bucket $\sign[\privKey](\bucket{\bucketIdx})$. The signatures are blinded with the random elements $\genericSig[\bucket{\bucketIdx}] \assign \sign[\privKey](\bucket{\bucketIdx})^{(\blindingFactor_{\bucketIdx})^{-1}} = \qrGen^{\frac{1}{\blindingFactor_\bucketIdx \prod\limits_{\genericBlocklistElmt \in \bucket{\bucketIdx}}{\primeRO(\genericBlocklistElmt)}}}$

\item 
Each auditor $\auditor[\auditorIdx]$ signs the (hash of) i) the public key of the distributed signing algorithm, ii) the joint signatures on the blocklist partitions and iii) the joint signature on $\blocklist'$:  $\auditorSig{\auditorIdx} \assign \signGenSig[\auditorPrivKeyGenericScheme[\auditorIdx]](\hashFn(\pubKey,  \genericSig[\bucket{1}], \dots, \genericSig[\bucket{\numBuckets}], \genericSig[\blocklist']))$

\item The auditors publish $\{\pubKey, \blocklist', \genericSig[\blocklist'], \genericSig[\bucket{1}], \dots, \genericSig[\bucket{\numBuckets}], \auditorSig{1}, \dots, \auditorSig{\numAuditors}  \}$ 
...
\item \label{pre:authOPRF} \client downloads $\{\pubKey, \blocklist', \genericSig[\blocklist'], \genericSig[\bucket{1}], \dots, \genericSig[\bucket{\numBuckets}], \auditorSig{1}, \dots, \auditorSig{\numAuditors} \}$ published by the auditors, and aborts if $\verifyGenSig[\auditorPubKeyGenericScheme[\auditorIdx]] (\auditorSig{\auditorIdx}, \hashFn(\pubKey,  \genericSig[\bucket{1}], \dots, \genericSig[\bucket{\numBuckets}], \genericSig[\blocklist'])) \neq 1$. 
...
\item (\algComment{Fine-grained check}) If $\detected = \tru$, \server computes the buckets corresponding to the elements in the Bloom filter returned by \client: $\bucketIDSet \assign \{ \bucketIdx: \genericBlocklistElmt \in \bloomFilter[\bloomFilterIdx] ~\land ~\genericBlocklistElmt \in \bucket{\bucketIdx}\}$. \server sends \bucketIDSet to \client.  For each $\bucketIdx \in \bucketIDSet$

\begin{enumerate}
\item \label{upload:fine_grained:token_gen} \client computes $\randomize[\pubKey](\genericSig[\bucket{\bucketIdx}] \sigMinus[\pubKey] \genericCInputElmt) = 
\langle  \genericSig[\bucket{\bucketIdx}]^{\genericRand \primeRO(\genericCInputElmt)} \bmod \rsaMod, \ro(\qrGen^{\genericRand} \bmod \rsaMod) \rangle$, and sends the content verification token $\contentToken{\clientData}^{(\bucketIdx)} \assign  \genericSig[\bucket{\bucketIdx}]^{\genericRand \primeRO(\genericCInputElmt)} \bmod \rsaMod$, and a blinded encryption key $\fileKey' \assign \fileKey \oplus \ro(\qrGen^{\genericRand} \bmod \rsaMod)$ to \server. 

\item \label{upload:verify_sig_decrypt} For each $\genericBlocklistElmt \in \bucket{\bucketIdx}$, compute $\accSet_{\genericBlocklistElmt} \assign \prod\limits_{\genericFnInput \in \bucket{\bucketIdx} \setminus \{\genericBlocklistElmt\}}\primeRO(\genericFnInput)$. \server  tries to decrypt $\encrypt[\fileKey](\clientData)$  with the key $\genericSymmKey' \assign \ro\left(\left(\contentToken{\clientData}^{(\bucketIdx)}\right)^{\blindingFactor_{\bucketIdx} \accSet_{\genericBlocklistElmt}} \bmod \rsaMod\right) \oplus \fileKey'$. (Note that $\genericSymmKey' = \fileKey ~\text{if} ~\genericCInputElmt \in 
		\blocklist$ and \server obtains \clientData.)

\item If the decryption is successful for some $\genericBlocklistElmt \in \bucket{\bucketIdx}$, then \server outputs 
...
%
%\item \label{upload:verify_sig} Otherwise, \server computes $\bucket{\bloomFilterIdx} \assign \{ \genericBlocklistElmt: \genericBlocklistElmt \in \blocklist ~\land ~\genericBlocklistElmt \in \bloomFilter[\bloomFilterIdx] \}$ 
%
%
%\enumListStart
%\item For each $\genericBlocklistElmt \in \bucket{\bloomFilterIdx}$, compute $\accSet_{\genericBlocklistElmt} \assign \prod\limits_{\genericFnInput \in \bucket{\bloomFilterIdx} \setminus \{\genericBlocklistElmt\}}\primeRO(\genericFnInput)$. 
%
%
%
%\item \label{upload:verify_sig_decrypt} For each $\genericBlocklistElmt \in \bucket{\bloomFilterIdx}$, \server  tries to decrypt $\cOutputFile$ with the key $\genericSymmKey' \assign \ro(\cOutputMeta^{\accSet_{\genericBlocklistElmt}} \bmod \rsaMod)$ \quad \algComment{$\clientData = 
%		\decrypt[\genericSymmKey'](\cOutputFile) ~\text{if} ~\genericCInputElmt \in 
%		\blocklist$ }
%
%\item If the decryption is successful for some $\genericBlocklistElmt \in \bucket{\bloomFilterIdx}$, then \server outputs 
