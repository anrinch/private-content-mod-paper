...
\input{exact_match_protocol.tex}
...
\smallskip\noindent\textbf{Inputs:} The auditors have a common list of item $\blocklist \subseteq \universe$ 

%and public-private key pair $\{ \auditorPubKeyOurScheme[\auditorIdx], \auditorPrivKeyOurScheme[\auditorIdx] \}_{\auditorIdx \in \nats[\numAuditors]}$ for signature scheme \ourSigScheme

\smallskip\noindent\textbf{Functionality:}

\itemListStart
\item Sample and store a public-private key pair $\pubKey, \privKey$ for the signature scheme \ourSigScheme. Send a secret share $\langle \privKey\rangle_{\auditorIdx}$ to auditor $\auditor[\auditorIdx]$
\item Upon receiving $(\distSign, \auditor[\auditorIdx], \blocklist)$ from $\auditor[\auditorIdx]$, store $\blocklist$. After receiving the $\distSign$ message from all auditors, output $\sign[\privKey](\blocklist)$ if all messages have the same input $\blocklist$. Otherwise abort.  
\item Upon receiving $(\distAdd, \auditor[\auditorIdx], \blocklist, \genericSetVar)$ from $\auditor[\auditorIdx]$, store $\blocklist, \genericSetVar$. After receiving the $\distAdd$ from all auditors, output $\sign[\privKey](\blocklist \cup \genericSetVar)$ 
if all the messages have the same inputs $\blocklist, \genericSetVar$. Otherwise abort. 
\item Upon receiving $(\distDel, \auditor[\auditorIdx], \blocklist, \genericSetVar)$ from $\auditor[\auditorIdx]$, store $\blocklist,\genericSetVar$. After receiving the $\distDel$ from all auditors, output $\sign[\privKey](\blocklist \setminus\genericSetVar)$ if all the message have the same inputs $\blocklist, \genericSetVar$. Otherwise abort. 
\itemListEnd
\end{mdframed}
\end{figure}
\fi 
%The protocol was later extended by \citet{ong2005optimizing} to support a malicious minority of signing parties, and we will use this version to realize \distSigningFunc.

\iffalse
\myparagraph{Realizing \distSigningFunc } We can realize \distSigningFunc by extending our RSA-based signature scheme. The full protocol is in \figref{fig:protocol:dist_sign} in \appref{}. Recall that the public key of the signature scheme is the modulus $\rsaMod$ and a generator $\qrGen$, and the private key is the factorization of $\rsaMod$. To support distributed signing, we use the protocol of \citet{algesheimer2002efficient}\footnote{To the best of our knowledge this is the only protocol that ensures that $\rsaMod$ is generated from safe primes, which is necessary to ensure that $\qrGroup$ is cyclic} to generate a shared modulus $\rsaMod = (2 \sgPrime[1] + 1) (2 \sgPrime[2] + 1)$ such that the auditors only have additive shares of $\sgPrime[1], \sgPrime[2]$.  
 The shares allow the auditors to securely compute $\rsaMod$ and $\orderFn(\qrGroup) = \sgPrime[1] \sgPrime[2]$ using invocations of the BGW protocol. \rsaMod is included in the public key while the shares of $\orderFn(\qrGroup)$ are distributed to the auditors and kept as the private key. The generator selection is also performed using the BGW protocol by checking that for a random value $\qrGen \in [2, \rsaMod -1]$, selected by the auditors, $\qrGen^{\sgPrime[1]} \not \equiv 1 \bmod \rsaMod ~\land ~\qrGen^{\sgPrime[2]} \not \equiv 1 \bmod \rsaMod$. The protocol finds a random generator within $\bigO{\secParam}$ tries with overwhelming probability. 
While the key generation is the most expensive step of our protocol, \emph{this setup is only performed once when the auditors initialize the system}.

%It is necessary to perform this check without revealing the outputs of   
%$\qrGen^{\sgPrime[1]}, \qrGen^{\sgPrime[2]}$ in the clear, and thus we use the BGW instantiations. We introduce several optimizations to reduce the number of BGW invocations by trivially excluding cases where $\qrGen \notin \qrGroup$ without interactions among the auditors (by locally checking the Jacobi symbol) and ensure that within $\bigO(\secParam)$ tries, the protocol identifies a random generator with overwhelming probability. 

\myparagraph{Signing and updating signatures}
%
The function $\distSign$ used to sign a set of items $\blocklist$ is significantly cheaper.  We leverage the protocol of \citet[Fig. 3]{catalano2000computing} to compute an inverse modulo a shared order, $\prod\limits_{\genericBlocklistElmt \in \blocklist}\primeRO(\genericBlocklistElmt)^{-1} \bmod \sgPrime[1] \sgPrime[2]$. Knowing a share $\langle \genericSetElmt \rangle_{\auditorIdx}$, the auditors can compute $\sign[\privKey](\blocklist) = \qrGen^{\left(\sum\limits_{\auditorIdx \in [1, \numAuditors]}\langle \genericSetElmt \rangle_{\auditorIdx}\right)}\bmod \rsaMod = \prod\limits_{\auditorIdx = 1}^{\numAuditors} \qrGen^{\langle \genericSetElmt_{\setIdx} \rangle_{\auditorIdx}} \bmod \rsaMod$. Crucially, this product is computed \emph{in the clear} after each auditor broadcasts  $\qrGen^{\langle \genericSetElmt \rangle_{\auditorIdx}} \bmod \rsaMod$. The costs scale polynomially in the size of $\blocklist$. Concretely, it takes 166 seconds to sign a blocklist with 1 million items (see \secref{sec:eval}). 
...
%
Given a blocklist, the auditors distribute the items into \numBuckets disjoint buckets. Then using \distSigningFunc, they sign each bucket separately to generate \numBuckets blocklist authentication (sub)tokens $\blklistToken{\blocklist} \assign \{ \blklistToken{\blocklist}^{(1)}, \dots, \blklistToken{\blocklist}^{(\numBuckets)}\}$, where $\blklistToken{\blocklist}^{(\bucketIdx)} \assign \sign[\privKey](\bucket{\bucketIdx})$. The partitioning makes the verification during file uploads more efficient since the entire blocklist is not checked, as described later. The auditors add a random number to each bucket to prevent offline dictionary attacks against the signature; this a common trick to hide the items in cryptographic accumulators~\cite{li2007universal}. 
...
% The client uses the token to derive the bin $\bloomFilterIdx \assign \genericCInputElmtPRFEval \bmod \numBuckets$ in which the hash resides, if it exists (\stepref{upload:bloom_filter_idx}). The client sends the encrypted file, \genericCInputElmtPRFEval and $\encrypt[\genericSymmKey](\bloomFilter[\bloomFilterIdx])$ to the server. 
%
...
If $\prp{\prpKeyServer}(\genericCInputElmtPRFEval) \in \bloomFilter[\bloomFilterIdx]$, the server performs a fine-grained check. The server first determines all the items $\genericBlocklistElmt \in \bloomFilter[\bloomFilterIdx]$, and their corresponding buckets. Assuming that the server has also precomputed and locally stored a Bloom filter for each bucket $\bloomFilter[\bucket{\bucketIdx}]$, this is equivalent to computing the items in the Bloom filter intersections $\bigcup_{\bucketIdx \in \nats[\numBuckets]} \bloomFilter[\bloomFilterIdx] \cap \bloomFilter[\bucket{\bucketIdx}]$ and a set of their corresponding bucket indices \bucketIDSet. The set of indices is sent to the client to perform a fine-grained check. (If the client does not respond, then the file is deleted). 



After receiving \bucketIDSet, the client uses the locally-stored signature $\genericSig[\bucket{\bloomFilterIdx}]$ for each $\bucketIdx \in \bucketIDSet$ to derive the content verification token 
$\contentToken{\clientData}^{(\bucketIdx)}$, and a one-time pad for the key encrypting the uploaded file, using the \emph{randomizing token} from the output of $\randomize[\pubKey](\genericSig[\bucket{\bloomFilterIdx}] \sigMinus[\pubKey] \genericCInputElmt)$ (\stepref{upload:fine_grained:token_gen}). If $\genericCInputElmt \in \blocklist$, then the server obtains the randomizing token by computing $\ro(\contentToken{\clientData}^{(\bucketIdx)} \sigMinus[\pubKey] \{\bucket{\bucketIdx} \setminus \{\genericBlocklistElmt\})$ for all $\genericBlocklistElmt \in \bucket{\bucketIdx}$ (\stepref{upload:verify_sig_decrypt}). In this case, the server outputs \clientData. Otherwise, if $\genericCInputElmt \notin \blocklist$, then the key remains hidden due to the security guarantees of \ourSigScheme, and random key robustness of the encryption scheme.  
...
\iffalse 
\begin{proofsketch}
We show in \appref{} that both the client's and server's views can be simulated. In brief, the client's view includes the blinded blocklist $\blocklist'$, and the signatures $\genericSig[\bucket{1}], \dots, \genericSig[\bucket{\numBuckets}]$. Each element in $\blocklist'$ is of the form $\oprfRO[1](\genericBlocklistElmt)^{\genericRand}$ which is a random value in the $\group$. The signatures 
...
$\genericCInputElmt \notin \blocklist$. During a fine-grained check, the server gets the content verification token $\contentToken{\clientData}^{(\bucketIdx)}$ for each $\bucketIdx \in \bucketIDSet$. If $\genericCInputElmt \notin \blocklist$, then each content verification token does not divulge any information to the server about $\genericCInputElmt$ due to the security guarantees of \ourSigScheme. Also, each invocation of $\randomize[\pubKey](\genericSig[\bucket{\bucketIdx}] \sigMinus[\pubKey] \genericCInputElmt) = \langle  \genericSig[\bucket{\bucketIdx}]^{\genericRand \primeRO(\genericCInputElmt)} \bmod \rsaMod, \ro(\qrGen^{\genericRand} \bmod \rsaMod) \rangle$ uses independent randomness, ensuring that the outputs are independent. Since $\ro$ is a random oracle $ \ro(\qrGen^{\genericRand_1} \bmod \rsaMod)$ and $ \ro(\qrGen^{\genericRand_2} \bmod \rsaMod)$ are uniformly random values. Further, with high probability  $\genericSig[\bucket{\bucketIdx}]^{\primeRO(\genericCInputElmt)} \bmod \rsaMod$ is a generator of \qrGroup, and thus $(\genericSig[\bucket{\bucketIdx}]^{\primeRO(\genericCInputElmt)})^{\genericRand_1} \bmod \rsaMod$ and 
$(\genericSig[\bucket{\bucketIdx}]^{\primeRO(\genericCInputElmt)})^{\genericRand_2} \bmod \rsaMod$ are uniformly distributed in \qrGroup~\cite{cramer2000signature}. 
\end{proofsketch}
\fi 

\myparagraph{Asymptotic costs}
%
The preprocessing phase requires a one-time $\bigO(\blocklistSize)$ compute costs and communication for the client and the server. The client also requires $\bigO(\blocklistSize^{1/\bloomFilterEpsilon} + \numBuckets)$ storage to store the encrypted Bloom filters and the $\numBuckets$ blocklist authentication token. During uploads, the client and server costs are as follows
...
\item $\bigO((\blocklistSize/\numBuckets)^{1/\bloomFilterEpsilon} + \log \blocklistSize)$ communication cost for sending an encrypted Bloom filter, and $\bigO(\log \blocklistSize)$ content verifications tokens if a fine-grained check is deemed necessary. 
\itemListEnd

\iffalse 
Specifically, the client computes $\randomize[\pubKey](\genericSig[\bucket{\bloomFilterIdx}] \sigMinus[\pubKey] \genericCInputElmt)$ which includes a randomized signature on $\bucket{\bloomFilterIdx} \setminus \{\genericCInputElmt\}$ if $\genericCInputElmt \in \bucket{\bloomFilterIdx}$ (\stepsref{upload:verif_token_1}{upload:verif_token_2}). The client uses the \emph{randomizing token} from the output of $\randomize[\pubKey](\genericSig[\bucket{\bloomFilterIdx}] \sigMinus[\pubKey] \genericCInputElmt)$ to encrypt the file. 
