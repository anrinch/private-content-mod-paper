...
\smallskip\underline{For every output of Type 2 $\langle \genericFileID, \detected = \fals, \bucketIDSet \rangle$}
...
\item If $\bucketIDSet \neq \emptyset$, then the simulator sets $\hat{\partitionEncryption{1, \projIdx}} \assign \pkencrypt[\encryptPubKey](t_k)$ where $t_k \in \bucketIDSet$. Otherwise, it sets $\hat{\partitionEncryption{1, \projIdx}} \assign \pkencrypt[\encryptPubKey](\plaintext_{\projIdx, 2})$ where $\plaintext_{\projIdx, 3} \sample \plaintextSpace$. The simulator removes $t_k$ from \bucketIDSet. 

%and  where $t_k$ is an entry in $\bucketIDSet$
