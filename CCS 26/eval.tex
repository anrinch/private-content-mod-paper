...
%
\figref{fig:dis_sign} shows the compute costs for signing and publishing the blocklist. We set the number of partitions $\numBuckets$ such that each partition has at most 64 items. This  was fixed based on the time to verify a signature. Signing a blocklist with a million items takes roughly 170 seconds with up to 10 auditors
...
%
\figref{fig:cpu-preprocess} shows the compute time required for preprocessing the blocklist. Recall that this is a $\bigO(\blocklistSize)$ operation for both the client and the server. In our implementation, the OPRF of choice is the 2HashDH over curve25519\footnote{https://www.ietf.org/archive/id/draft-irtf-cfrg-ristretto255-00.html}. The number of bins in our implementation was fixed to $\blocklistSize/\log \blocklistSize$. For a blocklist with a million elements, preprocessing takes roughly 21 seconds for the client and 36 seconds for the server. The encrypted Bloom filters require less than 4MB of client storage.
...
%
We conducted experiments for our fuzzy matching protocol \protocolFM using a blocklist with random 256 bit vectors that simulate perceptual hashes. We instantiated the projection functions with the construction of \citet{chongchitmate2024approximate}. The blocklist was partitioned into \numBuckets such that there are 64 vectors per bucket. We set the number of projections to 20 which gives a false positive rate $\badFP \le 0.02$ and $\badFP \le 0.07$. We compared against the private approximate matching (PAMC) protocol~\cite{kulshrestha2021identifying} that only tolerates a semi-honest server and has a higher false negative rate of roughly $17\%$
...
%the signature $\genericSig[\bucket{j}]$ where  $j = \prf{\prfKey}(\genericCInputElmt) \bmod \numBuckets$. 
...
We compared \protocolFM against the PAMC protocol of \citet{kulshrestha2021identifying}, and \tblref{tab:comp} presents the results. We observe that in the average case, when the coarse-grained check does not identify a match, our protocol is faster (\textbf{by a factor of 5$\times$}) and requires less communication (\textbf{by at least a factor of 25$\times$}). However, the fine-grained check is more expensive. 
